...
public class LinkedList<T> {
    ...
    public void removeAtIndex(int i) {
        ...
        // removing first node, change head to be second node
        if(i == 0) {
            head = head.getNext();
            return;
        }
        ...
        Node headTemp = head;
        int count = 0;
        // loop till you get to right before node to be removed
        while(count < i - 1) {
            headTemp = headTemp.getNext();
            count++;
        }
        //set next to be the one after the node we are removing (remove pointer to node i)
        headTemp.setNext(headTemp.getNext().getNext());
    ...
    public boolean isSymmetrical() {
        ...
        int i = 0;
        while(i < end) {
            //comapare element at i and size - 1 - 1 and continue to make them get closer to eachother
            if(this.get(i).equals(this.get(end))) {
                i++;
                end--;
            } else
                return false;
    ...
    public void multiply(int factor) {
        ...
        Node headTemp = head;
        while(headTemp != null) {
            Node next = headTemp.getNext();
            for(int i = 1; i < factor; i++) {
                Node newNode = new NodeImpl(headTemp.getValue(), next);
    ...
    public void removeRepeats() {
        ...
        while(headTemp.getNext() != null) {
            if(!headTemp.getValue().equals(next.getValue())) {
                headTemp = next;
                next = headTemp.getNext();
            } else {
                headTemp.setNext(next.getNext());
                next = headTemp.getNext();
            }
        }
    ...
    public boolean containsCycle() {
        ...
        while(headTemp != null) {
            visited.add(headTemp);
            if(visited.contains(headTemp.getNext())) {
                return true;
            }
            headTemp = headTemp.getNext();
